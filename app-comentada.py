...
def obtenerRespuestas(img):
    ...
    #hallar el objeto con mayor area y poner en blanco
    ...
    #seleccionar el objeto con mayor area
    maximaE = np.argmax(stats[:,4][1:])+1
    mascara = np.uint8(255*(maximaE==labels))

    #encontrar el convexHull para tener menos ruido
    ...
    #hallar los vertices y despues la homografía
    vertices = cv2.goodFeaturesToTrack( #emcuentra las esquinas mas prominentes en la imagen
                    mascaraCH,  #imagen
                    4,          #retorno maximo de esquinas
                    0.1,       #
                    20)
    ...
    #region de interes
    #se obtenie con porcentaje por si la resulución cambia
